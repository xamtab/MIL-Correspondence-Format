...
function buildWordDocHTML() {
    ...
    if (prec) body += `<p style="margin: 0 0 4pt 250pt; font-family:Arial,sans-serif; font-weight:bold; font-size:12pt; color:#cc0000;">${prec}</p>\n`;
    if (copyVal) body += `<p style="margin: 0 0 2pt 250pt; font-family:Arial,sans-serif; font-size:12pt;">Copy No ${copyVal}</p>\n`;
    if (pagesVal) body += `<p style="margin: 0 0 2pt 250pt; font-family:Arial,sans-serif; font-size:12pt;">Total Pages ${pagesVal}</p>\n`;

    // --- Sender Address ---
    const senderLines = sender.split('\n').map(l => l.trim()).filter(l => l);
    if (senderLines.length > 0) {
        body += `<p style="margin: 8pt 0 16pt 250pt; font-family:Arial,sans-serif; font-size:12pt; line-height:1.4;">${senderLines.join('<br>')}</p>\n`;
    }

    // --- Reference Number and Date (Ref left, Date aligned with sender via tab) ---
    body += `<p style="margin: 0 0 16pt 0; text-indent: 0; mso-tab-count: 1; tab-stops: 250pt; font-family:Arial,sans-serif; font-size:12pt;">`;
    ...
    // --- Paragraphs (Body) ---
    letterParagraphs.forEach((para, idx) => {
        body += `<p style="margin: 0 0 12pt 0; text-align: justify; text-indent: -36pt; margin-left: 36pt; tab-stops: 36pt; font-family:Arial,sans-serif; font-size:12pt;">`;
        body += `<span style="font-weight:500;">${idx + 1}.</span><span style="mso-tab-count:1">\t</span>${para.text}`;
        body += `</p>\n`;

        if (para.subParas.length > 0) {
            para.subParas.forEach((subText, subIdx) => {
                const subLetter = String.fromCharCode(97 + subIdx);
                body += `<p style="margin: 0 0 6pt 36pt; text-align: justify; text-indent: -36pt; margin-left: 72pt; tab-stops: 72pt; font-family:Arial,sans-serif; font-size:12pt;">`;
                body += `${subLetter}.<span style="mso-tab-count:1">\t</span>${subText}`;
                body += `</p>\n`;
            });
        }
    });
    ...
    // --- Signature Block ---
    body += `<p style="margin: 40pt 0 2pt 250pt; font-family:Arial,sans-serif; font-size:12pt; font-weight:bold; text-transform:uppercase;">${sigName.toUpperCase()}</p>\n`;
    body += `<p style="margin: 0 0 2pt 250pt; font-family:Arial,sans-serif; font-size:12pt;">${sigRank}</p>\n`;
    body += `<p style="margin: 0 0 24pt 250pt; font-family:Arial,sans-serif; font-size:12pt;">${sigAppt}</p>\n`;
